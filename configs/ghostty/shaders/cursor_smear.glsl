...
const float DURATION = .1; // seconds
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // ---- Dynamic smear color ----
    vec3 bg = fragColor.rgb;
    vec3 inverted = 1.0 - bg;

    // Tint color (bluish shift) — change to taste
    vec3 tint = vec3(0.9, 0.5, 0.2);

    // Mix inverted background with tint (0.25 = 25% tint)
    vec3 smearRGB = mix(inverted, tint, .5);

    // Final smear color with alpha
    vec4 TRAIL_COLOR = vec4(smearRGB, 1);
    // -----------------------------
